...
import {UserApi, UserProfile, UserProfileResponse} from "@src/codegn";
...
export const AuthContextProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [nrUser, setNrUser] = React.useState<UserProfile | undefined>(undefined);
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        new UserApi(headerConfig(token)).getUserprofile().then(response => {
          ...
          setNrUser(profileResponse.profile)
        }).catch(e => ERROR(e))
  ...
  return (
      <AuthContext.Provider value={{ authUser, nrUser }}>
